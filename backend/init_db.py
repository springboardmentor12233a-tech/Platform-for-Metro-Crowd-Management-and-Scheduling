...
def init_db():
    ...
    # Create Alerts Resolution Table
    ...
    # Create Emergency Announcements Table (Milestone 3 requirement)
    ...
    # Import datasets from CSV using pandas if available
    ...
    # Seed initial emergency announcement if none exist
    cursor.execute("SELECT COUNT(*) FROM emergency_announcements")
    if cursor.fetchone()[0] == 0:
        cursor.execute("""
        INSERT INTO emergency_announcements (title, message, line, station, severity, broadcast_by)
        VALUES ('Peak Hour Crowd Warning', 'High passenger surge detected on Yellow Line between Rajiv Chowk and New Delhi. Trains operating at 3 min headways.', 'Yellow Line', 'Rajiv Chowk', 'WARNING', 'admin')
        """)
        conn.commit()
