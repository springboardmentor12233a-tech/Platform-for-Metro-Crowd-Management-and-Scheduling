...
export default function Analytics() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [forecast, topStations, hourly, crowd] = await Promise.all([
          apiService.getForecast(),
          apiService.getTopStations(15),
          apiService.getHourlyPattern(),
          fetch('http://localhost:8000/api/crowd/all-stations')
            .then(res => res.json())
        ]);
  ...
  const trendData =
    data?.forecast?.forecast?.slice(0, 24)?.map((item: any) => ({
      hour: `${item.hour}:00`,
      passengers: item.predicted_passengers,
    })) || [];

  const hourlyData =
    data?.hourly?.hourly_pattern?.slice(0, 24)?.map((item: any) => ({
      hour: `${item.hour}:00`,
          ...
          <div className="space-y-3 text-slate-300">
            ...
            <p>
              ...
              <span className="font-bold text-green-400 ml-2">
                {(data?.forecast?.accuracy * 100).toFixed(1)}%
              </span>
            </p>
            ...
            <p>
              ...
              <span className="text-cyan-400 ml-2">
                Increase train frequency during peak hours to handle predicted demand.
              </span>
            </p>
