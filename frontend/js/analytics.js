...
async function loadAnalyticsPage() {
    try {
        ...
        if (data.stations) {
            ...
            if (selectEl) {
                selectEl.innerHTML = data.stations.map(s => `
                    <option value="${s.id}">${s.name} (${s.line})</option>
                `).join('');
            }
...
function renderStationComparisonChart(stations) {
    ...
    analyticsBarChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: stations.map(s => {
                    ...
                    if (s.current_density >= 40) return '#4FACFE';
                    return '#10B981';
                }),
                borderRadius: 6
            }]
        ...
        options: {
            ...
            scales: {
                x: { ticks: { color: '#9CA3AF' }, grid: { color: 'rgba(255,255,255,0.05)' } },
                y: { max: 100, ticks: { color: '#9CA3AF' }, grid: { color: 'rgba(255,255,255,0.05)' } }
            }
        }
    });
}
    ...
    analyticsDoughnutChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: ['#10B981', '#4FACFE', '#F59E0B', '#EF4444'],
                borderWidth: 2,
                borderColor: '#111827'
            }]
        },
